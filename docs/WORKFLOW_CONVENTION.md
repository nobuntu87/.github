...
| テンプレート | ラベル | 用途 |
| --- | --- | --- |
| `epic.yml` | `type:epic` | 大項目（複数の機能をまとめる単位） |
| `feature.yml` | `type:feature` | 機能単位（Epicを構成する単位） |
| `task.yml` | `type:task` | 作業単位（Featureを構成する単位） |
| `bug-report.yml` | `type:bug` | 不具合報告（予期しない動作・エラーの報告） |
| `enhancement.yml` | `type:enhancement` | 改善提案（機能・仕様・構成などの改善） |
...
- PRはレビュワー1名以上の承認を必須とします
- レビュー依頼はPRが完成した時点で行います（ドラフトPRの段階では依頼しません）
- レビュワーはコメントから3営業日以内に対応します
...
- GitHubのRelease自動生成機能を使用します
- 自動生成後に必要に応じて加筆・修正します
- `BREAKING CHANGE` がある場合は冒頭に明記します

### 5-4. リリース手順

```text
1. release/{バージョン} ブランチを main から作成
2. バージョン番号の更新（package.json など）
3. PR作成 → レビュー → Squash merge
4. GitHub Releases からタグ・リリースノートを作成
5. release ブランチを削除
```
